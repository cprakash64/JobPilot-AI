...
def configuration_summary(config: Settings = settings) -> dict[str, bool | str]:
    ...
    provider_configured = (
        bool(config.apollo_api_key)
        if provider == "apollo"
        else bool(config.pdl_api_key)
        if provider == "pdl"
    ...
    return {
        ...
        "global_budget_configured": config.people_daily_credit_budget > 0,
        "per_user_budget_configured": config.people_per_user_daily_limit > 0,
